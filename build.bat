...
:POST_ARGS_LOOP
%MSBUILD% %~dp0\DistroLauncher.sln /t:%_MSBUILD_TARGET% /m /nr:true /p:Configuration=%_MSBUILD_CONFIG%;Platform=x64 /p:AppxBundlePlatforms=x64 /p:AppxPackageDir=AppxPackages_x64 /p:AppxBundle=Always

if (%ERRORLEVEL%) == (0) (
    echo.
    echo Created appx in %~dp0x64\%_MSBUILD_CONFIG%\AlmaLinuxOS-8\
    echo.
)
