...
output_file=AlmaLinux-Kitten-10_x64_v2_"${build_version}".wsl

wsl_builder_ct=$(buildah from --platform=linux/amd64/v2 quay.io/almalinuxorg/almalinux:10-kitten)
...
buildah copy --chmod 0644 "$wsl_builder_ct" rootfs/wsl-distribution_kitten_10_x64_v2.conf /rootfs/etc/wsl-distribution.conf
...
wsl_img=$(buildah commit --squash --rm --manifest wsl:kitten_10 "$wsl_ct" wsl:kitten_10-x64_v2)

# Extract RootFS from the container image
rm -rfv wsl_kitten_10_x64_v2_dir && mkdir wsl_kitten_10_x64_v2_dir

buildah push "$wsl_img" oci:wsl_kitten_10_x64_v2_dir

manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_kitten_10_x64_v2_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_kitten_10_x64_v2_dir/blobs/sha256/"$manifest")

printf 'Root filesystem: %s\n' "$rootfs"

cp -v wsl_kitten_10_x64_v2_dir/blobs/sha256/"$rootfs" "$output_file"

sha256sum "$output_file" > "${output_file}".sha256sum

# Cleanup
rm -rfv wsl_kitten_10_x64_v2_dir
