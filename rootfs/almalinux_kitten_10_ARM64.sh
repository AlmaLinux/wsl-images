...
output_file=AlmaLinux-Kitten-10_ARM64_"${build_version}".wsl

wsl_builder_ct=$(buildah from --platform=linux/arm64 quay.io/almalinuxorg/almalinux:10-kitten)
...
buildah copy --chmod 0644 "$wsl_builder_ct" rootfs/wsl-distribution_kitten_10.conf /rootfs/etc/wsl-distribution.conf
...
# Note: systemd-tmpfiles-setup.service is not masked to allow creation of /run directories (e.g., for PostgreSQL)
...
wsl_img=$(buildah commit --squash --rm --manifest wsl:kitten_10 "$wsl_ct" wsl:kitten_10-ARM64)

# Extract RootFS from the container image
rm -rfv wsl_kitten_10_ARM64_dir && mkdir wsl_kitten_10_ARM64_dir

buildah push "$wsl_img" oci:wsl_kitten_10_ARM64_dir

manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_kitten_10_ARM64_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_kitten_10_ARM64_dir/blobs/sha256/"$manifest")

printf 'Root filesystem: %s\n' "$rootfs"

cp -v wsl_kitten_10_ARM64_dir/blobs/sha256/"$rootfs" "$output_file"

sha256sum "$output_file" > "${output_file}".sha256sum

# Cleanup
rm -rfv wsl_kitten_10_ARM64_dir
