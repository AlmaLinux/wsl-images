...
wsl_img=$(buildah commit --squash --rm --manifest wsl:9 "$wsl_ct" wsl:9-amd64)

# Extract RootFS from the container image
timestamp=$(date -u '+%Y%m%d')

rm -rfv wsl_9_amd64_dir && mkdir wsl_9_amd64_dir

buildah push "$wsl_img" oci:wsl_9_amd64_dir

manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_9_amd64_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_9_amd64_dir/blobs/sha256/"$manifest")

printf 'Root filesystem: %s\n' "$rootfs"

cp -v wsl_9_amd64_dir/blobs/sha256/"$rootfs" rootfs_wsl_9_amd64_"$timestamp".tar.gz

rm -rfv wsl_9_amd64_dir
