...
output_file=AlmaLinux-8."${minor_version}"_ARM64_"${build_version}".wsl

wsl_builder_ct=$(buildah from --platform=linux/arm64 quay.io/almalinuxorg/almalinux:8)
...
buildah copy --chmod 0644 "$wsl_builder_ct" rootfs/wsl-distribution_8.conf /rootfs/etc/wsl-distribution.conf
...
wsl_img=$(buildah commit --squash --rm --manifest wsl:8 "$wsl_ct" wsl:8-ARM64)

# Extract RootFS from the container image
rm -rfv wsl_8_ARM64_dir && mkdir wsl_8_ARM64_dir

buildah push "$wsl_img" oci:wsl_8_ARM64_dir

manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_8_ARM64_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_8_ARM64_dir/blobs/sha256/"$manifest")

printf 'Root filesystem: %s\n' "$rootfs"

cp -v wsl_8_ARM64_dir/blobs/sha256/"$rootfs" "$output_file"

sha256sum "$output_file" > "${output_file}".sha256sum

# Cleanup
rm -rfv wsl_8_ARM64_dir
