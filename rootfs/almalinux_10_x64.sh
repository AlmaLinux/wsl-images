...
output_file=AlmaLinux-10."${minor_version}"_x64_"${build_version}".wsl

wsl_builder_ct=$(buildah from quay.io/almalinuxorg/almalinux:10)
...
buildah copy --chmod 0644 "$wsl_builder_ct" rootfs/wsl-distribution_10.conf /rootfs/etc/wsl-distribution.conf
...
wsl_img=$(buildah commit --squash --rm --manifest wsl:10 "$wsl_ct" wsl:10-x64)

# Extract RootFS from the container image
rm -rfv wsl_10_x64_dir && mkdir wsl_10_x64_dir

buildah push "$wsl_img" oci:wsl_10_x64_dir

manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_10_x64_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_10_x64_dir/blobs/sha256/"$manifest")

printf 'Root filesystem: %s\n' "$rootfs"

cp -v wsl_10_x64_dir/blobs/sha256/"$rootfs" "$output_file"

sha256sum "$output_file" > "${output_file}".sha256sum

# Cleanup
rm -rfv wsl_10_x64_dir
