...
manifest=$(jq -r '.manifests[].digest | split(":")[1]' wsl_kitten_10_x64_dir/index.json)

rootfs=$(jq -r '.layers[] | select(.mediaType == "application/vnd.oci.image.layer.v1.tar+gzip") | .digest | split(":")[1]' wsl_kitten_10_x64_dir/blobs/sha256/"$manifest")
